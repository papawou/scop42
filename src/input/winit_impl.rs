...
pub enum Input {
    Key(winit::keyboard::PhysicalKey), // winit::event::WindowEvent::KeyboardInput;
...
//                 device_id, event, ..
//             } => Ok(DeviceInput(Some(device_id), Input::Key(event.physical_key))),
...
// impl StateAPI<winit::keyboard::PhysicalKey> for WinitInputManager {
//     fn is_press(&mut self, input: winit::keyboard::PhysicalKey) -> Option<Instant> {
//         self.keys
//             .get(&DeviceInput(None, Input::Key(input)))
...
//     fn is_release(&mut self, input: winit::keyboard::PhysicalKey) -> Option<Instant> {
//         self.keys
//             .get(&DeviceInput(None, Input::Key(input)))
//             .and_then(|state| match state {
//                 State::Released(instant) => Some(instant.clone()),
...
//     fn try_press(&mut self, input: winit::keyboard::PhysicalKey) -> Option<State> {
//         self.keys
//             .entry(DeviceInput(None, Input::Key(input)))
//             .or_insert(State::Pressed(Instant::now()))
//             .press()
//     }

//     fn try_release(&mut self, input: winit::keyboard::PhysicalKey) -> Option<State> {
//         self.keys
//             .entry(DeviceInput(None, Input::Key(input)))
//             .or_insert(State::Released(Instant::now()))
//             .release()
//     }

//     fn press(&mut self, input: winit::keyboard::PhysicalKey) -> Option<State> {
//         self.keys
//             .entry(DeviceInput(None, Input::Key(input)))
//             .or_insert(State::Pressed(Instant::now()))
//             .press()
...
//     fn release(&mut self, input: winit::keyboard::PhysicalKey) -> Option<State> {
//         self.keys
//             .entry(DeviceInput(None, Input::Key(input)))
//             .or_insert(State::Released(Instant::now()))
//             .release()
